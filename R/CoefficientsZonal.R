        ...
        for(j in (i+1L):n){
          for(t in seq_len(lambda[j])){
            ...
            if(isDominated(muOrd, kappa)){
              x <- x + (mu[i]-mu[j]) /
                (.rho(as.bigq(kappa))-.rho(as.bigq(lambda))) *
      ...
      for(i in 1L:(n-1L)){
        for(j in (i+1L):n){
          for(t in seq_len(lambda[j])){
            ...
            if(isDominated(muOrd, kappa)){
              x <- x + (mu[i]-mu[j]) / (.rho(kappa)-.rho(lambda)) *
...
zonalCoefficients <- function(n, until = NULL, exact = FALSE){
  if(exact){
    zonalCoefficientsQ(n, until)
  }else{
    zonalCoefficientsNum(n, until)
  }
}
